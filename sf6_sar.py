...
# 1) SAR parameter: log10(k_OH) for SF6 (cm³ molecule⁻¹ s⁻¹ at 298 K)
...
# 2) SMARTS pattern to recognize the SF6 fragment
SAR_SMARTS = {
    'SF6': '[S]([F])([F])([F])([F])([F])[F]',
}
...
# 4) count groups and sum log10(k)
log10_k_total = 0.0
for name, patt in patterns.items():
...
# 5) compute lifetime τ = 1 / (k_OH × [OH])
